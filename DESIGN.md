...
- 会话流固定 336px；顶部 44px 行只保留 Heading 16 上下文标题与右侧垂直居中的 icon-only 排序按钮，不重复展示会话总数；整组与窗口标题栏和详情顶行共用中心线，行下留 8px 再开始会话。排序按钮与详情工具栏统一为 16px 图标、透明 ghost 常态、6px 圆角，当前排序字段和方向放在 tooltip 与菜单选中态中。
