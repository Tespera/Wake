...
- 会话流固定 336px；顶部的 22px 上下文标题与 icon-only 排序按钮共享一个 44px 高的 Flex 行，与窗口标题栏和详情顶行共用垂直中心；Label 11 会话数量保留在标题下方。排序按钮与详情工具栏统一为 16px 图标、透明 ghost 常态、6px 圆角，当前排序字段和方向放在 tooltip 与菜单选中态中。
