...
- 顶部由 22px 上下文标题、会话总数角标和 icon-only 排序按钮组成；整个标题区固定为 88px，并拆成两个 44px 行：标题与排序按钮对应左栏窗口控制区，会话总数对应 `Wake` 品牌行；第二行以 8px 顶部光学补偿抵消小字号行框，使字形与 `Wake` 落在同一水平带。排序按钮与详情工具栏统一为 16px 图标、透明 ghost 常态、6px 圆角，当前排序字段和方向放在 tooltip 与菜单选中态中。
