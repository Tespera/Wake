...
| 最小窗口 | 940 × 620 |
| 窗口顶部 | 内容区使用贯穿三栏的 34px chrome；macOS 交通灯嵌入左段，Windows 原生标题栏保留在其外，Linux 视 compositor 装饰协商而定 |
| 资料库侧栏 | 224px 固定宽度 |
| 会话流 | 336px 固定宽度 |
| 阅读区 | 剩余宽度，`min_w(0)`；正文最大宽度 720px |

窗口内容顶部先绘制一条统一的 34px chrome：左段承接交通灯与 Wake，中段承接当前范围、会话数和排序，右段承接来源上下文与当前会话操作；三段共享高度、材质、底部分隔和拖拽区域。chrome 下方的侧栏、会话流、详情头和正文再分别使用 `sidebar`、`list`、`background`、`popover` 材质表达层级，不加投影。
...
| token | 浅色 | 深色 | 用途 |
|---|---:|---:|---|
| `title_bar` | `#EDEDEA` | `#1B1B1A` | 贯穿三栏的窗口顶部 chrome |
...
内容区顶部固定为 34px：三栏均使用 `title_bar` 材质、同一条底部 hairline 和垂直中心线。macOS 左段复用真实 TitleBar，把 Wake 放在交通灯之后；中段依次放当前范围与会话数，排序靠右；右段依次放 Agent、项目、分支，当前会话操作靠右。未选择会话时右段仍保留空 chrome，不能让顶栏在中栏末端突然断开。Windows 保留系统原生标题栏（贴靠布局与深色模式经 DWM 处理），内容区下方仍使用同一条 34px chrome；Linux 由 compositor 装饰协商决定，报 Client 时左段使用自绘 TitleBar。

### 资料库侧栏

- 顶部 chrome 在交通灯后显示 Wake；其下是唯一的全文搜索入口,文案 "Search sessions",右侧显示 `⌘K`;Search/All Sessions/Starred 固定不随滚动。
...
- 会话流固定 336px；顶部 chrome 使用 Body 14 semibold 上下文标题、Label 11 会话数量和 icon-only 排序按钮，三者单行垂直居中。排序按钮与详情工具栏统一为 16px 图标、透明 ghost 常态、6px 圆角，当前排序字段和方向放在 tooltip 与菜单选中态中。
- 会话行使用 Body 14 标题与 Label 11 元信息，共两行；行内 `SPACE_SM` 上下内边距 + `SPACE_XS` 两行间距。
- 第二行固定为品牌图标 15px、项目名 badge、弹性空隙和右对齐相对时间。
...
1. 34px 顶部 chrome 中的 Agent、项目、分支等来源上下文与右侧操作工具条；两组必须处于同一个 Flex 行，不使用绝对定位或负偏移。
2. 独占整行、最多两行的 22px 会话标题。
3. 独占一行的项目路径。
4. 模型、来源 badge 与消息数、token 组成的统计行。
5. 精确到秒的创建时间与更新时间；两者作为同一组从信息区左边界依次排列。

五层信息一项不减。第一层属于全窗 chrome，后四层属于独立的详情信息区；层内保持紧凑，层间至少 8–12px，让来源、标题、位置、运行统计和时间上下文能被分别扫读。
