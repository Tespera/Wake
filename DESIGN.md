...
| 最小窗口 | 940 × 620 |
| 窗口顶部 | macOS 不设显性全宽标题栏；三栏共同遵守 34px 窗口净空，交通灯与真实 TitleBar 仍只在侧栏；Windows 用原生标题栏，Linux 视 compositor 装饰协商而定 |
| 资料库侧栏 | 224px 固定宽度 |
| 会话流 | 336px 固定宽度 |
| 阅读区 | 剩余宽度，`min_w(0)`；正文最大宽度 720px |

窗口不绘制全宽标题栏。侧栏承接交通灯、窗口拖拽区和唯一的全文搜索入口；会话流与阅读区保持各自材质延伸到窗顶，只用无形净空追平侧栏的 34px TitleBar。三栏首个可见标题内容都从窗顶 42px（34px 净空 + 8px 内容间距）开始，不额外绘制同色带、底线或容器。侧栏、会话流、详情头和正文分别使用 `sidebar`、`list`、`background`、`popover` 材质表达层级，不加投影。
...
macOS 不设置横跨三栏的自定义 header。系统交通灯保留在侧栏顶部，其所在拖拽区与侧栏使用完全相同的材质和颜色；会话流与阅读区只在自身 header 内增加同高的透明顶部净空，背景仍分别属于 `list` 与 `background`，不得添加贯穿三栏的色带或 hairline。Windows 保留系统原生标题栏（贴靠布局与深色模式经 DWM 处理）；Linux 由 compositor 装饰协商决定，报 Client 时侧栏顶部挂自绘 TitleBar。
...
- 顶部由 22px 上下文标题、会话总数角标和 icon-only 排序按钮组成；排序按钮与详情工具栏统一为 16px 图标、透明 ghost 常态、6px 圆角，当前排序字段和方向放在 tooltip 与菜单选中态中。
...
1. Agent、项目、分支等来源上下文与右侧操作工具条。
2. 独占整行、最多两行的 22px 会话标题。
3. 独占一行的项目路径。
4. 模型、来源 badge 与消息数、token 组成的统计行。
5. 精确到秒的创建时间与更新时间；两者作为同一组从信息区左边界依次排列。

五层信息一项不减，但不可压回同一横行。层内保持紧凑，层间至少 8–12px，让来源、标题、位置、运行统计和时间上下文能被分别扫读。
