...
3. 模型与来源（Via）badge 独占标题下方第一行。
4. 消息数与 token 独占第二行。
5. 项目路径独占第三行。
6. 第四行以 12px 日期图标开头，随后依次排列精确到秒的创建时间与更新时间。

六层信息一项不减；标题下方的四行信息继续保持独立层级，不压进 88px 标题区域。层内保持紧凑，层间至少 8–12px，让来源、标题、运行统计、位置和时间上下文能被分别扫读。
